...
func TestCSRFTokenManager(t *testing.T) {
	...
	t.Run("rejects expired token", func(t *testing.T) {
		...
		err = manager.ValidateToken(sessionID, token)
		if err != ErrExpiredCSRFToken && err != ErrInvalidCSRFToken {
			t.Errorf("expected ErrExpiredCSRFToken or ErrInvalidCSRFToken (after cleanup), got %v", err)
		}
	})
